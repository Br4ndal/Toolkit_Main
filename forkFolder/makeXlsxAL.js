...
console.log(content);
  ...
  for (let i = 0; i < dataAI3.length; i++) {
    ...
    let alarmInfoString = new String(alarmInfo) // making a object of the variable alarmInfo
    let alarmInfoUpper = alarmInfoString.toString().toUpperCase() // then converting it to string and then to upper case

    let IASTagNameString = new String(IASTagName)
    let IASTagNameStringUpper = IASTagNameString.toString().toUpperCase()


    if (SystemName == "PMS" && (alarmInfoUpper == "YES" || alarmInfoUpper == "MON")) {
      console.log(typeof (SystemName), SystemName, typeof (alarmInfo), alarmInfo);
      pmsAlmNr = pmsAlmNr.concat(IASTagNameStringUpper);
      pmsSfi = pmsSfi.concat(tagName)
    }
  }
  console.log("PMS alarms");
  console.log(pmsAlmNr);
  ...
  */
  ...
  let test = [];
  let pmsNewFianlArrayPage = newFianlArrayPage
  console.log(newFianlArrayPage);
  for (let i = 0; newFinalArrayAlmNr.length > i; i++) {

    for (let j = 0; pmsAlmNr.length > j; j++) {
      //console.log(typeof (newFinalArrayAlmNr[i]), typeof (pmsAlmNr[j]), newFinalArrayAlmNr[i], pmsAlmNr[j], j);
      if (newFinalArrayAlmNr[i] == pmsAlmNr[j] && pmsNewFianlArrayPage[i] == '') {
        //console.log(typeof (newFinalArrayAlmNr[i]), typeof (pmsAlmNr[j]), newFinalArrayAlmNr[i], pmsAlmNr[j], j);
        pmsNewFianlArrayPage[i] = "M_MNU_4_01"
      }
    } //newFinalArrayAlmNr.length
  ...
  XLSX.utils.sheet_add_aoa(ws, arrayFliperRowtoColumn(newtankSystemAlarmsSB), { origin: "AC1" });
  XLSX.utils.sheet_add_aoa(ws, arrayFliperRowtoColumn(tankSystemAlarmNrSB), { origin: "AD1" });
  XLSX.utils.sheet_add_aoa(ws, arrayFliperRowtoColumn(tankSystemAlarmsSB), { origin: "AE1" });

  XLSX.utils.sheet_add_aoa(ws, arrayFliperRowtoColumn(pmsNewFianlArrayPage), { origin: "AF1" });
